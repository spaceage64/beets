        ...
        for batch in chunks(to_migrate, 100):
            ...
            with self.db.transaction() as tx:
                tx.mutate_many(
                    ...
                    [(lyr.full_text, _id) for lyr, _id in ids_with_lyrics],
                )
                tx.mutate_many(
                    f"""
                    INSERT INTO {flex_table} (entity_id, key, value)
                    VALUES (?, 'lyrics_backend', ?),
                           (?, 'lyrics_url', ?),
                           (?, 'lyrics_language', ?),
                ...
                tx.mutate(
                    f"""
                    DELETE FROM {flex_table}
                    WHERE key IN ('lyrics_backend', 'lyrics_url', 'lyrics_language', 'lyrics_translation_language')
                    AND value IS NULL
                    """  # noqa: E501
                )
